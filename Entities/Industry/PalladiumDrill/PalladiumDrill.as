...
const u16 heat_add = 2;
const u16 heat_add_constructed = 2;
const u16 heat_add_blob = 8;
const u16 heat_cool_amount = 2;

const u16 heat_cooldown_time = 8;
const u16 heat_cooldown_time_water = u16(heat_cooldown_time / 2);

const f32 max_heatbar_view_range = 130;
...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		if (int(heat) >= heat_max - (heat_add * 1.5))
		{
			...
			if (getGameTime() % 10 == 0) this.server_Hit(holder, holder.getPosition(), Vec2f(), 0.25f, Hitters::burn, true);
		...
		if (true)
		{
			...
			u16 delay_amount = 6;
			if (this.get_bool("just hit dirt")) delay_amount = 8;
					...
					if (map.getHitInfosFromArc((this.getPosition() - attackVel), -attackVel.Angle(), 30, distance, this, true, @hitInfos))
					{
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							else // map
							{
								...
								if (tiletypecustom > 529 && tiletypecustom < 535)
								{
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u16 customData)
{
	...
	{
		s16 current_heat = this.get_u16(heat_prop) - heat_max*0.7f;
