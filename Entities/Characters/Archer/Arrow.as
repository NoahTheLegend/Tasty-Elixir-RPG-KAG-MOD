...
{
	if (cmd == this.getCommandID("senddamage"))
	{
		f32 dmg = params.read_f32();
		u16 shooterid = params.read_u16();
		CBlob@ blob = getBlobByNetworkID(shooterid);
		blob.set_f32("dealtdamage", dmg);
	}
}*/
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		else
		{
			...
			if (shooter !is null) 
			{
				f32 damage = dmg - (dmg * (blob.get_f32("damagereduction")/10));
				damage = Maths::Round(damage*1000);
				damage /= 1000;
				shooter.set_f32("dealtdamage", damage);
			}
			/*CBitStream@ params;
			if (params is null) return;
			params.write_f32(dmg);
			params.write_u16(this.get_u16("shooternetid"));
			this.SendCommand(this.getCommandID("senddamage"), params);*/
		}
